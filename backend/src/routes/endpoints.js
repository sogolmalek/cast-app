...
router.post('/generate', authenticate, async (req, res) => {
  ...
  // Get user's API key + provider config
  const user = db.prepare('SELECT claude_api_key_encrypted, ai_provider, ai_model FROM users WHERE id = ?').get(req.user.id);
  if (!user?.claude_api_key_encrypted) {
    return res.status(400).json({ error: 'API key required. Add your key in Settings.' });
  }
  ...
  try {
    ...
    // Generate endpoint code (with provider + model)
    const spec = await generateEndpoint(prompt, user.claude_api_key_encrypted, history, user.ai_provider || 'anthropic', user.ai_model);
...
router.post('/iterate', authenticate, async (req, res) => {
  ...
  try {
    ...
    const spec = await iterateEndpoint(instruction, currentCode, user.claude_api_key_encrypted, history, user.ai_provider || 'anthropic', user.ai_model);
