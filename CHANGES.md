...
- **`adapters/index.js`** — registers `solana`, `base`.
...
- **`Balance.jsx`** — withdrawal defaults to Solana; dropdown now offers Solana (AUDD) / Base (USDC); address placeholder updated.
- **`Landing.jsx`** — copy now advertises AUDD settlement on Solana; chain row shows Solana + AUDD badge.
- **`Settings.jsx`** — supported-chains panel lists Solana (native) with AUDD features, plus Base.
...
- **`backend/.env.example`** — Solana/AUDD block replaces the Starknet block; Base block unchanged.
- **`docker-compose.yml`** — Starknet env vars out, Solana env vars in.
...
- `GET /cast/chains` returns `solana, base`.
